...
def create_user(username:str, login:bool, retry = 100):
    ...
    password_hash = bcrypt.hashpw(password.encode("utf-8"), authz_db_salt) if login else None
...
def api_key_from_login(username:str, password:str):
    password_hash = password_hash = bcrypt.hashpw(password.encode("utf-8"), authz_db_salt)
    connection = sqlite3.connect('authorization.db')
    with connection:
        cursor = connection.cursor()
        cursor.execute('''
